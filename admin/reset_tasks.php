...
// 检查用户是否已登录并且是管理员
if (!isset($_SESSION['is_logged_in']) || !$_SESSION['is_logged_in'] || !isSuperAdmin($_SESSION['role'] ?? '')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reset_tasks'])) {
    try {
        ...
        foreach ($tasks as $task) {
            ...
            // 重置任务主状态为待处理
            $db->debugQuery("UPDATE todos SET status = 'pending', updated_at = datetime('now', 'localtime') WHERE id = ?", [$taskId]);
            
            // 重置该任务的所有子工序状态为待处理
            $db->debugQuery("UPDATE todo_steps SET status = 'pending', updated_at = datetime('now', 'localtime') WHERE todo_id = ?", [$taskId]);
